...
**FR-16 — Grid export (added post-launch, Sprint 10).** The user can export their weekly grid as Markdown (every block they own, grouped by day), SVG (a static image matching the on-screen grid, including collision/out-of-range clamping), PNG (a client-side canvas render of the live grid), or a printed/PDF page (via the browser's native print-to-PDF). Not part of this PRD's original scope (§13 Sprints 1-9) — added afterward by explicit user request, with an explicit zero-new-runtime-dependency constraint. See §13 Sprint 10.
...
- [x] **7.1 Palette management**
    - [x] 7.1.1 `BlockColorForm` (name + `<input type='color'>` synced to hex text field) with validation. *(The native `<input type="color">` companion is hand-rolled directly in `color_form_row.html`, not a Django widget — mirrors the Sprint 6 color-swatch precedent (Django's generic field rendering has no hook for the shared `data-hex-sync` attribute `static/js/color-sync.js` needs to pair the two inputs). `BlockColorForm.clean()` also hand-rolls a `(user, name)` uniqueness check: `user` is excluded from `Meta.fields`, so Django's automatic `UniqueConstraint` validation never fires for it during `full_clean()` — verified by tracing the installed Django 6.0.7 source (`code-reviewer`), not assumed. Without this, a duplicate name would 500 with an `IntegrityError` instead of a clean inline validation error.)*
    - [x] 7.1.2 Palette panel in dashboard toolbar: HTMX-rendered list of swatches with add/edit/delete (CBVs: `ColorCreateView`, `ColorUpdateView`, `ColorDeleteView`), all user-scoped. *(Same whole-fragment-swap philosophy as Sprint 6: every color CRUD success re-renders the whole `#palette-panel`, not a per-row patch. Also carries an out-of-band `#grid-table` refresh in the same response — a color rename/recolor/delete changes how existing blocks render, so `_render_palette_response()` refreshes both fragments in one round trip via a new `grid_oob` context flag on `grid_table.html`, not a follow-up request. Two small extra views not named in the PRD's literal task list: `PaletteView` (GET, restores the "+ Add color" trigger) and `ColorCancelView` (GET, restores a color's display row) — the palette-panel analogs of Sprint 6's `CellCancelView`/`BlockCancelView`, needed for the same reason.)*
    - [x] 7.1.3 Verify SET_NULL behavior: deleting a color leaves blocks styled with the neutral/default block appearance. *(Verified end-to-end, not just at the DB level: `qa-tester` confirmed a color deletion's response carries the OOB grid refresh showing the affected block's colorless fallback styling immediately, in the same response, not merely eventually-consistent until a later page load.)*
    - [x] 7.1.4 Refresh color choices in open block forms after palette changes (simple approach: forms fetch fresh on open). *(Already satisfied by Sprint 6's own design, no new code needed: `TimeBlockForm.__init__` already re-queries `BlockColor.objects.filter(user=user)` on every instantiation, and the block form is always GET-fetched fresh, never cached client-side. Verified directly this sprint rather than assumed.)*
- [x] **7.2 Theming and visual QA**
    - [x] 7.2.1 Full dark/light audit of every screen and fragment (including HTMX partials rendered standalone). *(`django-frontend` found and fixed two real contrast gaps predating this sprint — the footer's `text-slate-500` with no dark variant (~3.74:1, under WCAG AA) and `block_form.html`'s "no color" `×` glyph (~2.56:1, already flagged as a known deferred item in Sprint 6's ARCHITECTURE.md notes) — plus audited every new Sprint 7 palette template. No Playwright in this environment; audit method is the same hand-verified class-string/contrast-math approach `code-reviewer` already used successfully in Sprints 4-6.)*
    ...
    - [x] 7.2.3 Responsive audit: landing, auth, dashboard grid at mobile/tablet/desktop breakpoints. *(Two real, pre-existing bugs found and fixed: (1) the grid table's `table-fixed w-full` inside its `overflow-x-auto` wrapper could never actually overflow, so narrow viewports silently squeezed all 7 day columns unreadably thin instead of scrolling — fixed with a `min-w-[760px]` floor on the `<table>`. (2) the dashboard toolbar's two `<details>` dropdowns (Settings, Palette, this sprint) each declared their own `relative` positioning context, so Settings' panel would anchor to Settings' own right edge and overflow off-screen now that Palette sits to its right — fixed by hoisting `relative` to their shared row wrapper, plus adding a shared `name="dashboard-toolbar"` attribute so the two `<details>` behave as an exclusive accordion (native HTML behavior, no JS, per NFR-01). Both fixes reasoned through by hand, not browser-confirmed — see the Playwright gap below.)*
- [x] **7.3 Quality pass**
    - [x] 7.3.1 PEP 8 / single-quote sweep (run `ruff`/`flake8` locally, config committed; keep it a dev-only tool). *(Added `ruff` as a `uv` dev-dependency group member (`pyproject.toml`'s `[dependency-groups] dev`, not the runtime `dependencies` list — the KPI's "≤5 entries in `requirements.txt`" budget is untouched) with `[tool.ruff]`/`[tool.ruff.lint]` config (E/F/W/I, migrations excluded from line-length since they're generated, not hand-written). `ruff check` found 6 genuinely dead `F401` imports in Django's default `startapp` stub files (`admin.py`/`models.py`/`tests.py` across `accounts`/`core`/`planner`) and fixed them. Deliberately did **not** run `ruff format` across the repo: its own `--diff` preview would have reverted the Sprint 6 PEP 701 single-quote-nesting fix (`f'Skipped {', '.join(...)}...'`) back to mixed quotes, and would have reflowed large amounts of this codebase's deliberate, hand-crafted multi-line docstrings/formatting — a bulk restyle was never this task's intent. `ruff check .` is the enforced dev command; `[tool.ruff.format] quote-style = 'single'` stays configured for anyone formatting new code per-file going forward.)*
    - [x] 7.3.2 Remove dead code/templates; ensure all strings and UI copy are English. *(`code-reviewer`'s explicit sweep: no stray TODO/FIXME/dead-placeholder markers found; no non-English strings found (only pre-existing typographic symbols: `§`, em-dashes, curly quotes); confirmed the old "Palette is still an inert placeholder" dashboard comment language was fully replaced, not just the button it described. The one real finding — the PRD §9.2 Secondary-button class string hand-duplicated across 6 call sites in 5 templates, crossing the threshold Sprint 6's ARCHITECTURE.md had already pre-flagged — was treated as blocking and fixed immediately: extracted `templates/partials/buttons/secondary.html`, mirroring `primary.html`'s dual-mode shape plus an `as_summary` mode for the toolbar `<details>` triggers and named `hx_get`/`hx_target`/`hx_swap` context variables (no `|safe`/raw-attribute escape hatch) for the two HTMX Cancel buttons; all 6 call sites migrated, byte-for-byte behavior parity verified via test client.)*
    - [x] 7.3.3 Manual regression checklist executed (auth, settings, all block operations, palette, themes). *(`qa-tester` ran this plus a full adversarial pass on the new palette feature — uniqueness/hex validation, ownership isolation across all 5 new endpoints, SET_NULL + immediate-grid-refresh correctness, `HX-Retarget` precision, cancel-flow data freshness, toast-channel isolation, the `grid_oob` flag's Sprint-6-regression safety, and `repeat_days` interacting with a real palette color for the first time — zero defects found. Method: Django test client throughout, no Playwright in this environment (see gaps section).)*
...
- [x] **9.1 Docker**
    - [x] 9.1.1 Write `Dockerfile` (slim Python base, install requirements, collectstatic, run via `gunicorn` added to requirements for container only — or `runserver` for dev image, documented). *(`python:3.12-slim`; `gunicorn`/`whitenoise` added as a new `docker` `[dependency-groups]` entry in `pyproject.toml` — not the main `dependencies` list, which stays at just `django` — mirroring Sprint 7's `dev`/`ruff` pattern, since this project uses `uv`/`pyproject.toml` rather than `requirements.txt` (Sprint 1 deviation). `collectstatic --noinput --ignore=input.css` runs at image build time (`--ignore` needed because the raw, uncompiled Tailwind v4 source file isn't a real static asset — confirmed load-bearing by `code-reviewer`, who reproduced the exact `MissingFileError` it prevents); `migrate --noinput` runs at container *start* instead, since it depends on the runtime-mounted volume. All uv/whitenoise specifics confirmed via Context7, not guessed — see `docs/ARCHITECTURE.md`.)*
    - [x] 9.1.2 Write `docker-compose.yml` (app service, volume for SQLite file, env file support). *(Deviation: the named volume is mounted onto a dedicated directory, `/app/data`, not directly onto the `db.sqlite3` file — a direct file-path mount was tried first and fails outright on this Docker daemon with "is not directory" on first creation, reproduced independently by both the implementing agent and `code-reviewer`. `config/settings.py`'s `DATABASES` now reads an `SQLITE_DB_PATH` env var, set only inside the Dockerfile; local dev is unaffected. `env_file: .env` uses the Compose long-form `required: false` syntax so a fresh clone without a `.env` yet doesn't hard-fail — confirmed current, non-deprecated syntax via Context7. See `docs/ARCHITECTURE.md`.)*
    - [x] 9.1.3 Verify: fresh clone → `docker compose up` → migrate → app reachable; document the exact commands. *(Verified independently three times: by the implementing agent, by this session directly (build → up → curl landing/static/admin/login → SQLite-persistence-across-`down`/`up` test → down), and by `code-reviewer` (same cycle, plus a live repro of the `collectstatic --ignore` requirement and a Context7 cross-check of the WhiteNoise middleware placement). `qa-tester` additionally ran the full 82-test suite *inside* the running container plus a full HTTP-level manual regression pass — see 9.3.1.)*
- [x] **9.2 Documentation**
    - [x] 9.2.1 Write `README.md`: overview, screenshots, features, local setup (venv + Tailwind build), Docker setup, project structure, design tokens summary. *(Deviation: no live-app screenshots — no browser-screenshot tooling is available in this environment, an already-documented gap since Sprint 1 (see "Known, expected gaps" in `docs/ARCHITECTURE.md`). The repo's existing pre-development concept sketch, `assets/preview.svg`, is embedded instead and explicitly labeled as a concept sketch, not a live screenshot, so nobody mistakes it for one.)*
    - [x] 9.2.2 Write `CONTRIBUTING`/customization notes: how to add an app, swap palette, change grid defaults; note SQLite limits and how to switch DB settings. *(`CONTRIBUTING.md`; cross-checked against the actual `apps/planner/models.py`/`apps/planner/apps.py` code by `code-reviewer`, not just written from memory of the spec.)*
    - [x] 9.2.3 Add `LICENSE` (open-source) and finalize `.env.example` docs. *(Deviation: initially MIT, then changed to the **PolyForm Noncommercial License 1.0.0** per explicit user instruction — personal/noncommercial use only, selling or any commercial use is not permitted. This is a deliberate departure from this task's own "open-source" framing and from PRD §1/§5/US-5.2's repeated "open-source template" language: PolyForm Noncommercial is source-available, not an OSI-approved open-source license, since it restricts commercial use. Full unmodified license text fetched verbatim from its canonical source rather than reproduced from memory. `.env.example` gained a short note cross-referencing `docker-compose.yml`'s optional `env_file` usage.)*
- [x] **9.3 Release**
    - [x] 9.3.1 Final regression run (manual checklist + full test suite) inside Docker. *(`qa-tester`: 82/82 automated tests passing inside the container via `docker compose exec web python manage.py test`, plus a full manual HTTP-level regression checklist against the running container — signup/login/logout, full block+color CRUD cycles, overlap rejection, touching-blocks-allowed, `SET_NULL`, the `grid_oob` OOB-refresh contract, and static-file serving verified with real response headers. Found and this session fixed one real, non-blocking gap: the README's Docker quickstart didn't warn that the no-`.env` default (`DEBUG=True`, empty `ALLOWED_HOSTS`) is dev-only — a warning was added. `code-reviewer`'s independent pass found zero blocking issues and two further small documentation-accuracy fixes (also applied): the "before building" wording was wrong (these env vars are read at container *start*, not build time) and the same paragraph didn't mention replacing the public repo's fallback `SECRET_KEY`. A non-root container user was suggested as a further hardening step and deliberately deferred — see `docs/ARCHITECTURE.md`.)*
    - [ ] 9.3.2 Tag `v1.0.0`; publish repository as the reusable template. *(Deliberately not done: this requires `git tag`/`git push`, and this entire session operates under a hard standing instruction to never commit or push. Left unchecked rather than falsely marked done — this is the one remaining step once the user reviews and explicitly authorizes it.)*

### Sprint 10 — Grid Export (Markdown, SVG, PNG, PDF) — post-launch addition, not in the original scope above

> Added entirely after Sprint 9 by explicit user request ("Add a option to export (PDF, PNG, SVG, markdown)"), not part of this PRD's original §13 plan. The user was asked, up front, to choose between a zero-new-dependency approach and a heavier server-rendered-file approach (with the trade-offs of each spelled out); they chose the zero-new-dependency path. See FR-16 and `docs/ARCHITECTURE.md`'s new "Grid export (Sprint 10)" section for the full design.

- [x] **10.1 Markdown export**
    - [x] 10.1.1 `apps.planner.export.render_week_markdown(blocks, time_format)`: pure function, one `## <Day>` heading per day (Monday–Sunday via `TimeBlock.DAY_CHOICES`, even for days with zero blocks — shown as `_No blocks._`), each block as a `- <start>–<end> <label>` bullet honoring the user's `time_format`. *(Real bug found by `qa-tester`'s adversarial pass and fixed: a label containing an embedded newline could inject a fabricated `## Heading`/bullet line into the exported document, landing under the wrong day. Fixed via `safe_label = ' '.join(block.label.split())` before formatting each bullet — collapses any embedded whitespace, including newlines, to single spaces. Regression test added.)*
    - [x] 10.1.2 `ExportMarkdownView` (`GET /planner/export/markdown/`, `LoginRequiredMixin`, scoped to `request.user`, no pk): returns `text/markdown; charset=utf-8` with `Content-Disposition: attachment; filename="weekly-planner.md"`.
- [x] **10.2 SVG export**
    - [x] 10.2.1 `apps.planner.export.build_svg_export(week_grid)`: pure function reusing the exact same `build_week_grid()` matrix the on-screen grid renders from (same collision re-anchoring, same out-of-range clamping), turning it into pixel-positioned `SvgRect`/`SvgText` dataclasses — the template that consumes them does zero layout math (PRD R2, same discipline as `grid.py`/`grid_table.html`). Same newline-normalization fix as 10.1.1 applied to block labels here too, for consistency (flagged by `code-reviewer`, not independently discovered by `qa-tester` — SVG's own default whitespace handling meant this wasn't a live bug, but the fix keeps a single invariant enforced the same way at both call sites).
    - [x] 10.2.2 `apps/planner/templates/planner/partials/grid_export.svg` + `ExportSVGView` (`GET /planner/export/svg/`, same auth/scoping as 10.1.2): returns `image/svg+xml`, `Content-Disposition: attachment; filename="weekly-planner.svg"`. Autoescaping deliberately left on (never `{% autoescape off %}`/`|safe`) since a block label is untrusted user text flowing into XML markup — verified well-formed via `xml.etree.ElementTree.fromstring()` against adversarial `&`/`<`/`>`/quote-carrying labels, not just eyeballed.
- [x] **10.3 PNG export (client-side, zero new dependencies)**
    - [x] 10.3.1 `static/js/export.js`: reads the live `#grid-table table` DOM (`getBoundingClientRect()`/`getComputedStyle()` per cell — background, border, and either the `.block-label` child's text or the cell's own text), draws it onto an off-screen `<canvas>` sized by `devicePixelRatio`, downloads via `canvas.toDataURL('image/png')`. No server endpoint, no image-rendering library.
- [x] **10.4 PDF export (client-side, zero new dependencies)**
    - [x] 10.4.1 Print-only Tailwind `print:` variant classes (`print:hidden` on the navbar/footer/dashboard toolbar; `print:max-h-none print:overflow-visible print:ring-0` on `#grid-table`'s wrapper) plus a "Print / Save as PDF" button calling `window.print()` — the browser's own native print-to-PDF, no PDF-generation library.
- [x] **10.5 Dashboard UI**
    - [x] 10.5.1 A third toolbar `<details name="dashboard-toolbar">` ("Export"), joining the existing Settings/Palette exclusive-accordion group: Markdown/SVG as plain `<a href>` downloads, PNG/Print as buttons wired to `export.js`. *(`code-reviewer` caught two real NFR-05 duplication regressions in the first draft — the toolbar panel's long wrapper class string hand-copied a 3rd time instead of factored out, and the PNG/Print buttons hand-copying `secondary.html`'s button class string instead of routing through it. Both fixed: the class string is now a single `{% with panel_class=... %}` value used by all three panels (a full new shared partial was judged disproportionate — this exact wrapper is only ever used inside this one file, unlike the button partials, which are genuinely cross-template); `secondary.html` gained two narrowly-named `data_export_png`/`data_export_print` flags, mirroring its existing `hx_get`/`hx_target`/`hx_swap` precedent, rather than a generic `data-*` passthrough.)*
- [x] **10.6 Verification**
    - [x] 10.6.1 `qa-tester` ran an adversarial HTTP-level pass beyond the implementing agents' own tests: full/empty weeks, mixed colored/colorless blocks, Markdown- and XML-special characters in labels, both `time_format`s including a midnight-crossing block, two-user ownership isolation, anonymous-auth redirects, and POST-method rejection on both new endpoints — found the newline-injection bug above (fixed) and nothing else. `code-reviewer` then independently re-verified the fix, the XML escaping, NFR-07 isolation, and NFR-01 (zero new dependencies — `pyproject.toml`/`uv.lock` untouched throughout), and found the two duplication issues above (fixed). PNG/print visual correctness remains unverified (no Playwright/browser in this environment) — noted as an addition to the existing gap in `docs/ARCHITECTURE.md`, not a new, separate one. Suite grew from 82 to 101 tests, all passing; `ruff check .`/`manage.py check`/`makemigrations --check --dry-run` all clean throughout.
